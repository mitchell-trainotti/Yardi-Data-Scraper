...
const executeLookups = async (pointers) => {
    ExportedData.push(['Property Code', 'Intercom Code', 'Front Entrance Code', 'LockBox Code', 'Boilter Room Access', 'Master Code', 'Lockbox Location 1', 'Lockbox Code 1', 'Lockbox Location 2', 'Lockbox Code 2', 'Lockbox Location 3', 'Lockbox Code 3', 'Notes']);
    for (pointer of pointers) {
        ...
        // checkPointer("bal_lock_propscode_TextBox", el, pointer);
        ...
        var AccessInfo = [propertyCode, intercomCode, frontEntranceAccess, lockBoxCode, boilerRoomCode, masterCode, box1Location, box1Code, box2Location, box2Code, box3Location, box3Code, notes];

        ExportedData.push(AccessInfo);
    }
    // goodPointers.splice(0, 99);
    // console.log(goodPointers);
...
function checkPointer (id, el, pointer){
    if (el.getElementById(id) != null){
        if (el.getElementById(id).value.length == 4){
            goodPointers.push(pointer);
        }
    }
}
